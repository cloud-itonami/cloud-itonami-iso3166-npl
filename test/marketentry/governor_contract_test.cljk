  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Kathmandu Civic Systems Pvt. Ltd."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Kathmandu Civic Systems Pvt. Ltd." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest investment-approval-authority-mismatch-is-held-and-unoverridable
  (testing "declared :claimed-investment-approval-authority does not match the independently recomputed FITTA 2075 दफा १७ tier -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:investment-approval-authority-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest ocr-registration-unverified-is-held-and-unoverridable
  (testing "unverified OCR company-registration certificate when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:ocr-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest pan-unverified-is-held-and-unoverridable
  (testing "unverified PAN (tax-ID) when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:pan-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
