...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Pvt. Ltd." :jurisdiction "NPL"
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "NPL" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:foreign-investment-npr (:engagement m)) (:foreign-investment-npr (:engagement d))))
    (is (= (:claimed-investment-approval-authority (:engagement m)) (:claimed-investment-approval-authority (:engagement d))))
