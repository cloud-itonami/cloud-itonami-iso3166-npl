(ns marketentry.marketentryllm
  ...
  the Nepal (NPL) public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real portal registration/
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world portal registration/filing
  submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "foreign-investment-npr=" (:foreign-investment-npr e)
                        " claimed-investment-approval-authority=" (:claimed-investment-approval-authority e)
                        " ocr-registration-verified?=" (:ocr-registration-verified? e)
                        " pan-verified?=" (:pan-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ocr-registration? e))
                              (:ocr-registration-verified? e))
                          (or (not (:requires-pan? e))
                              (:pan-verified? e)))
                   0.9 0.3)}))
