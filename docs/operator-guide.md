...
2. Register the client's intake: business type, target public function,
   prior filing history in Nepal if any.
3. Run the advisor in read-only mode against Bolpatra (bolpatra.gov.np),
   the PPMO-operated e-GP portal.
4. Compare the checklist against the client's current documentation
   (OCR registration certificate, PAN certificate).
...
- named referral relationship with Nepali-licensed counsel or a
...
a working referral relationship with Nepali-licensed counsel or a
registered agent for whatever licensed representation the law of
Nepal requires for actual public-procurement filings.
